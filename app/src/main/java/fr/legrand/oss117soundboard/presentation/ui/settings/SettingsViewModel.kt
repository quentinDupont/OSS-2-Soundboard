...
class SettingsViewModel @Inject constructor(
    ...
    fun listenToRandomReply() {
        contentRepository.retrieveRandomReplyIdToListen().subscribeOn(Schedulers.io()).subscribeBy(onNext = { listenToReply(it) }, onError = { it.printStackTrace() })
    }
    ...
    private fun getReplySort() {
        contentRepository.getReplySort().subscribeOn(Schedulers.io()).subscribeBy(onNext = { replySort.postValue(it) }, onError = { it.printStackTrace() })
    }
    ...
    private fun getTotalReplyTime() {
        contentRepository.getTotalReplyTime().subscribeOn(Schedulers.io())
                .subscribeBy(onNext = { totalReplyTime.postValue(Triple(it / MS_TO_H, it / MS_TO_M % M_S_MODULO_VALUE, it / MS_TO_S % M_S_MODULO_VALUE)) },
                        onError = { it.printStackTrace() })
    }
